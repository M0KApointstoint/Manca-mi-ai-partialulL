...
- 1.) *Motivatie* — De ce avem nevoie de o lista inlantuita?. Ce problema rezolva fata de array?.
    ...
    - Array-ul are dimensiune fixa declarata la inceput. Daca nu stim de la inceput
...
- 2.) *Ce este* — Cum arata o lista inlantuita in memorie?. Ce o defineste structural?.
    ...
    - O lista inlantuita este o colectie de noduri unde fiecare nod contine doua lucruri:
...
- 3.) *Operatii* — Ce poti face cu o lista inlantuita?. inserare, stergere, cautare, traversare:
    ...
    - Inserarea: cel mai eficient la inceput (la head), deoarece nu necesita traversare.
...
- 4.) *Complexitate* — Cat costa fiecare operatie ca timp si spatiu?.
    ...
    - Inserarea: O(1) la inceput, O(n) la mijloc sau la final; spatiu auxiliar O(1);
...
- 6.) *Cazuri de utilizare* — Cand alegi o lista inlantuita si de ce?.
    ...
    - Cand numarul de elemente variaza frecvent si nu este cunoscut la inceput;
